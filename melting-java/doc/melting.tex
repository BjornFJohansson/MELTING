...
\item [\textbf{-E} \textit{ion1\_name=x.xxe-xx:ion2\_name=x.xxe-xx:agent1\_name=x.xxe-xx...}] \mbox{}\\
  ...
  values and in mol/L. There are some exceptions for the DMSO concentrations (in %) and the formamide concentrations
  (in % or mol/L depending on the used correction method). Be aware, the $[\mbox{Tris}^+]$ is about half of the total tris buffer
  concentration.
...
\item [\textbf{-self}]\mbox{}\\
  ...
  examples:   
  \begin{verbatim}
  - The sequence ATCGCGAT is self complementary. The option \textbf{-self} is not necessary because the programm can automatically detect it.
  - The sequence -TCGCGAT is self complementary but with a single dangling end. The option \textbf{-self} is not necessary because the programm can automatically detect it.
  - If the sequence ATCCCGAT is self complementary with a single mismatch (C/C), the option \textbf{-self} is necessary to precise the self complementarity because the 
    programm can't automatically detect it.
  \end{verbatim}
...
\item [\textbf{-am} \textit{method\_name}]\mbox{}\\ 
  ...
  \item [\textit{DNA duplexes}]
    ...
    \end{itemize}
  \item [\textit{RNA duplexes}]
    \begin{itemize}
    \item [\textit{wetrna91}] (from Wetmur 1991)  (by default)
    \end{itemize}
  \item [\textit{DNA/RNA duplexes}]
    \begin{itemize}
...
\item [\textbf{-ion} \textit{method\_name}]\mbox{}\\ 
  ...
  \item [\textit{Sodium corrections}]
    ...
    \item [\textit{DNA duplexes}]
      ...
      \end{itemize}
    \item [\textit{RNA duplexes or mRNA/RNA duplexes}]
      \begin{itemize}
    ...
    \item [\textit{DNA/RNA duplexes}]
      ...
      \end{itemize}
    \end{itemize}
  \item [\textit{Magnesium corrections}]
    \begin{itemize}
    \item [\textit{DNA duplexes}]
      \begin{itemize}
    ...
    \item [\textit{RNA duplexes or mRNA/RNA duplexes}]
      ...
      \end{itemize}
    \end{itemize}
  \item [\textit{Mixed Na Mg corrections}]
    \begin{itemize}
    \item [\textit{DNA duplexes}]
      \begin{itemize}
